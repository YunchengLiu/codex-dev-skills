...
Most steps follow the dependency-based order. A clearly bounded step may proceed beside it when its purpose, inputs, boundary, result, and checks are already settled. Do not use shared feature membership as proof that a step should start. If later integration must decide its behavior or interface, leave it in the plan rather than creating a detailed brief. The kind of code being built does not decide whether work can start.

## Minimum Content

A useful brief contains only the applicable items:

1. **Goal and result.** State the behavior, capability, evidence, or decision completed by this step and how it advances the larger plan.
...
Do not introduce a new abstraction, compatibility layer, asynchronous path, cache, recovery scheme, or other machinery unless the current step or repo requires it. A future scale estimate supports avoiding obvious waste and hard-to-change limits; it does not make future optimization part of this step.
...
Make the step small enough that its behavior, edits, and verification can be reviewed together. It may map to one commit, several inseparable or adjacent commits, or part of a larger approved commit according to repo and user policy. If it is split, list the commits in order and state the real boundary. Commit count does not define the plan.
...
- Does the current step complete a concrete result rather than merely move a source-tree component?
- Are its result, prerequisites, edit boundary, and acceptance known without guessing later design?
