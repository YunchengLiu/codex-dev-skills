...
Review the current document against the confirmed goal, repo, and current design level. Report only issues with a plausible consequence for delivered behavior, architecture, work order, verification, scope, acceptance, or safe continuation. A review may correctly find no substantive issue; do not manufacture findings to demonstrate effort or novelty.
...
8. **Put content and decision order first.** Arrange the spec so the goal, whole-system view, parts, order and reasons, contracts, current detail, and acceptance follow naturally. Use plain domain language and only formatting, terms, and rationale that preserve a real decision. Do not turn the spec into a literary exercise, textbook, code commentary, or formal proof. (See `references/writing-principles.md`.)
...
10. **Correct the spec directly and keep records sparse.** When spec revision is authorized, correct wrong or unclear design in its defining section. Record progress only when a future session needs the changed state, and decisions only when an important accepted choice is not already clear in the corrected spec. Do not record conversation history, review chatter, private naming, or code-level nits. (See `references/progress-and-decisions.md`.)
