...
4. **Minimum sufficient, correctness-first design.** Treat the spec as one coherent design and establish the simplest maintainable implementation that is correct for the declared current domain. Add performance, scale, concurrency, asynchronous coordination, recovery, compatibility, extensibility, caching, or state machinery only when current acceptance, callers, lifecycle, data volume, failure handling, or integration risk requires it. Otherwise place evidenced optimization after functional correctness and keep speculative improvements out of the phase plan.
...
11. **Phase order preserves the whole executable design.** Derive every phase from the spec's overall behavior target and implementation spine. Each phase has one dependency-closed, review-sized responsibility with its own acceptance, verification, and handoff: everything required for that acceptance already exists or lands no later than the phase. Later phases may extend or optimize a correct current result, but must not be required to make an earlier phase's claimed behavior correct. Helper-first phases are valid only when an established caller surface already makes that work independently meaningful.
