...
### 3. Ask only a few high-leverage questions
...
### 4. Research external facts when they affect the plan
...
### 5. Make uncertainty explicit instead of guessing
...
- Once the plan is clear enough, produce a concise brief with goal, scope, non-goals, constraints, unknowns, recommended path, deliverables, and completion criteria.
...
### 8. Confirm who the plan is for if still unclear
...
### 9. Generate a fresh-context LLM prompt only when useful
...
- This skill is for clarifying and planning work, not for owning domain-specific implementation decisions end to end.
